...
- 大量に`interface`を書いてると、依存の中の依存でドキュメントされていない可能性がある
...
- `config.json`で指定した場合は入っていないものも検知する
- Freshで対応が入るかも
- Deno独自の実装
...
- 今後は[Wasmer JS](https://github.com/wasmerio/wasmer-js)などへの移行が推奨されている
  - こちらのほうが専門家なので、任せる感じ
- Node.jsモジュールの中にWASIモジュールを組み込むようにする話があったはず
  - Node.js互換性でWASIをやる場合は、opを使って地道に進める感じになりそう
