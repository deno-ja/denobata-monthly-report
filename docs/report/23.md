...
- Javaにも[try](https://docs.oracle.com/javase/tutorial/essential/exceptions/tryResourceClose.html)という似たようなやつがある
- Pythonに[with](https://docs.python.org/3.8/library/contextlib.html)という似たようなものがある
- JavaやPythonはネストが一つ増えるので、`using`のほうが便利そう
- 小文字のdisposablesは再代入できない<!-- TODO: GitHub上のusingの仕様の中にそれらしいのが見当たらず…… -->
...
- SemVerの解決をパッケージレジストリとランタイムで合わせて解決する<!-- TODO: すみません、聞き逃したので記載があやふやです -->
