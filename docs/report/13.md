...
- 利用するAPIが`--unstable`だったから雑だったのだが、`hostname()`をstableにする流れになり、合わせてパーミッションをちゃんとした
- kt3kさんが実装された
...
- プラグインシステム (例: Twindプラグイン)
  - UnoCSSはクラスの生成が非同期なんだけど、Freshが非同期フックに対応したら入れられる段階になってる
- Preact Signalsのサポート
  - 1.1
  - 24時間以内にFreshがサポートした
  - アイランドと相性がいい
    - アイランドは同じレンダリングツリーにいないので、Context APIが使えず難儀していた
- _middleware.tsで複数のミドルウェアが指定できるように改善
...
- Deno v2の予定はあったりする？
  - しばらく出ない
  - Node互換性の向上
  - Node互換の入れ方をnpm:を追加する形になってブレーキングチェンジじゃなくなった
  - v2というのを使う必要がなくなった
  - v2を度のタイミングで出すかという話がなくなってしまった
- Softwere Designに連載する話
  - kanehiraさんにお願いしようと思っている
  - 2〜3回で出してほしい
  - kt3kさんはいくつか記事を溜めているので着手されている
  - kanehiraさんに
  - 1月2月頃の予定
  - magurotunaさんは昔Web+DBプレスに書いた
  - 向こうから案は出してくれている
  - ページ数の指定は6〜8ページ
  - 1ページ1200文字くらい
  - コードスニペットやキャプチャは文字数食う
  - kanehiraさんはVimの記事を同雑誌に載せていたので編集さんは知ってた
  - 日本のVimコミュニティは活気がすごい
    - アドカレ新年突入とか
  - Golangコミュニティも活発
  ...
  - RyanさんもVimmer
  - 他のメンバーはVS Codeが多め
- Cache API がファイルベースの保存となっているのが気になっていて、deno deploy だとop?を差し替えている？って解釈であってるんでしょうか？
  - そうです
  - Opレベルで全部差し替えている
  - Cache APIにしてもユーザーの使い勝手は同じだけど、中身は全然違う
  - Denoで使う場合は内部でネットワークアクセスとか走る感じになっている
  - そのサーバーのファイルに保存とかじゃなくて各リージョンにレスポンスオブジェクトが格納されるような形になる
- 虎の穴開発室ブログで
